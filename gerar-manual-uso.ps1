...
Bul "Marcos atrasados: conta, entre os marcos de projetos ainda ativos no Relatorio de Situacao (secao 4.1), quantos ja passaram da data-alvo sem estar Concluido/Cancelado."
Bul "Projetos dentro do orcamento: compara, projeto a projeto, o total de Custos realizados (secao 4.1) com o total estimado no TAP daquele projeto. So entram na conta os projetos com projeto vinculado e ao menos uma categoria de custo realizado preenchida."
Bul "Entraves em aberto: mesma contagem do Painel Executivo, aqui no contexto da reuniao de resultados."
Img "51_relatorio_resultados_kpis.png" "Os tres indicadores estrategicos do Relatorio de Resultados." 5.8
Nota "Marcos atrasados nao e o mesmo que `concluido no prazo`: o sistema guarda a data-alvo do marco, nao uma data separada de quando ele foi de fato concluido - por isso o indicador mostra atraso atual, o dado mais honesto disponivel hoje. Da mesma forma, Projetos dentro do orcamento aparece como travessao (`-`) quando nenhum projeto ainda tem custo realizado registrado, em vez de mostrar um numero sem base."
