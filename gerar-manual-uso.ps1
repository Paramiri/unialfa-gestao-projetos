...
P "Versao 2.82 - 16 de setembro de 2026 (substitui a versao 2.81 de 15/09/2026)" 11 $false $false $colMuted "left" 2
...
P "Responsavel (conta) em cada risco: alem do campo de texto livre Responsavel, cada risco tem um segundo campo opcional, `Responsavel (conta)` - um seletor com as contas que fazem parte da equipe do projeto selecionado, mesmo padrao ja usado no cronograma do Planejamento (secao 6.4.4, Meu Painel). Quando preenchido, o risco passa a aparecer no Meu Painel (secao 6.4.4) daquela pessoa, com um botao para encerra-lo (muda o Status para Encerrado) direto de la."
...
P "Responsavel (conta): alem do campo de texto livre Responsavel, cada marco tem um segundo campo opcional, `Responsavel (conta)` - um seletor com as contas que fazem parte da equipe do projeto selecionado (secao 2.5). Diferente do texto livre, que e so uma anotacao, esse campo vincula o marco a uma conta de verdade: e a partir dele (mais o papel Gerente de Projetos da equipe do projeto) que o sistema sabe para quem mandar o aviso automatico de prazo (secao 6.4.3), para popular o Painel de Prazos (secao 6.4.2) e para popular o Meu Painel (secao 6.4.4) daquela conta. Preencher esse campo e opcional - um marco sem `Responsavel (conta)` continua funcionando normalmente, so nao gera aviso individual nem aparece no Meu Painel de ninguem."
P "Responsavel (conta) nas Entradas (aba 4): no mesmo padrao do cronograma acima, cada linha da aba Entradas tem um seletor opcional `Responsavel (conta)` com as contas da equipe do projeto. Quando preenchido, a entrada passa a aparecer no Meu Painel (secao 6.4.4) daquela pessoa, com um botao para conclui-la (muda o Status para OK) direto de la."
...
Nota "Responsavel (conta): diferente do mesmo campo no cronograma do Planejamento e no Registro de riscos do TAP (que listam so a equipe do projeto), aqui o seletor `Responsavel (conta)` lista todas as contas do sistema - Entraves e Encaminhamentos nao tem um projeto vinculado unico, entao nao ha uma equipe para restringir a lista. Quando preenchido, o item passa a aparecer no Meu Painel (secao 6.4.4) da conta selecionada, com um botao `Resolver` que muda o Status para Resolvido direto de la. E opcional."
...
Bul "Um cartao por pessoa, com nome, papel e a contagem de itens em cada uma das 5 categorias, mais quantos estao atrasados - clicar no cartao expande a lista detalhada de cada item, na mesma organizacao do Meu Painel. E somente visualizacao: nao ha botao para concluir um item em nome de outra pessoa."
